...
describe("admin SettingsView platform quota matrix", () => {
  ...
  it("保存时 updateSettings payload 应包含嵌套 default_platform_quotas 对象（含全部平台）", async () => {
    ...
    const platforms = ["anthropic", "openai", "gemini", "grok", "antigravity", "custom"];
  ...
  it("加载后 form.default_platform_quotas 含全部平台，从嵌套 JSON 正确读取数值", async () => {
    getSettings.mockResolvedValueOnce({
      ...
      default_platform_quotas: {
        ...
        openai:    { daily: null, weekly: 12.5, monthly: null },
        // gemini / grok / antigravity / custom 缺失 → 应被归一化为全 null
      },
    });
    ...
    expect(quotas["custom"]).toEqual({ daily: null, weekly: null, monthly: null });
  });
