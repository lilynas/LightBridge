...
describe('UserPlatformQuotaCell', () => {
  ...
  it('多平台按固定顺序展示 custom，未知平台排在最后，仅展示有限额的', () => {
    const w = mount(UserPlatformQuotaCell, {
      props: {
        quotas: [
          item({ platform: 'grok', monthly_limit_usd: 80 }),
          item({ platform: 'custom', monthly_limit_usd: 70 }),
          item({ platform: 'zeta-provider' as PlatformQuotaItem['platform'], monthly_limit_usd: 60 }),
    ...
    expect(text.indexOf('grok')).toBeLessThan(text.indexOf('custom'))
    expect(text.indexOf('custom')).toBeLessThan(text.indexOf('zeta-provider'))
