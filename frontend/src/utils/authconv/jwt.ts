/**
 * JWT utilities — ported from https://github.com/ltxgit/authconv
 * Handles decoding, synthetic token creation, and OpenAI claim extraction.
 */
...
export function createSyntheticIdToken(claims: Record<string, unknown>): string {
  const header = {
    alg: 'none',
    typ: 'JWT',
    cpa_synthetic: true
...
export function applySyntheticIdTokenSignature(token: string): string {
  const parts = token.split('.')
