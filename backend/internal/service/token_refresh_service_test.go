...
func TestTokenRefreshService_RefreshWithRetry_InvalidatesCache(t *testing.T) {
	...
	service := NewTokenRefreshService(repo, nil, nil, nil, invalidator, nil, cfg, nil)
	account := &Account{
		ID:       5,
...
func TestTokenRefreshService_RefreshWithRetry_NilInvalidator(t *testing.T) {
	...
	service := NewTokenRefreshService(repo, nil, nil, nil, nil, nil, cfg, nil)
	account := &Account{
		ID:       7,
		Platform: PlatformGemini,
		Type:     AccountTypeOAuth,
...
func TestTokenRefreshService_RefreshWithRetry_ClearsTempUnschedulable(t *testing.T) {
	...
	service := NewTokenRefreshService(repo, nil, nil, nil, invalidator, nil, cfg, tempCache)
