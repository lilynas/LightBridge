...
type opsBatchMarkReadRequest struct {
	IsRead bool `json:"is_read"`
}

// BatchMarkRequestErrorsRead batch-updates is_read for request errors matching filter criteria.
// PUT /api/v1/admin/ops/request-errors/mark-read
func (h *OpsHandler) BatchMarkRequestErrorsRead(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	...
	var req opsBatchMarkReadRequest
	...
	affected, err := h.opsService.BatchUpdateErrorReadStatus(c.Request.Context(), filter, req.IsRead)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
