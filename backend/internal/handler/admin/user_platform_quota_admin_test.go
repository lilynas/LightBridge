...
func TestUpdateUserPlatformQuotas_Success(t *testing.T) {
	...
	// 缓存失效：对全部允许平台统一 invalidate，新增平台后这里应自动同步。
...
func TestUpdateUserPlatformQuotas_AllAllowedPlatformsAccepted(t *testing.T) {
	repo := &upsertCapturingQuotaRepo{}
	cache := &billingCacheStub{}
	h := buildTestHandler(repo, cache)
	...
	for i, platform := range service.AllowedQuotaPlatforms {
		if i > 0 {
			b.WriteByte(',')
		}
		b.WriteString(`{"platform":"`)
		b.WriteString(platform)
		b.WriteString(`","daily_limit_usd":1}`)
	}
	b.WriteString(`]}`)

	c, w := putReq(t, b.String())
	h.UpdateUserPlatformQuotas(c)
	...
	if got := len(repo.upsertCalls[0].records); got != len(service.AllowedQuotaPlatforms) {
		t.Fatalf("expected %d quota records, got %d", len(service.AllowedQuotaPlatforms), got)
	}
}
...
func TestUpdateUserPlatformQuotas_RejectsTooManyEntries(t *testing.T) {
	h := buildTestHandler(&upsertCapturingQuotaRepo{}, &billingCacheStub{})
	body := `{"quotas":[
		{"platform":"anthropic"},{"platform":"openai"},{"platform":"gemini"},{"platform":"grok"},{"platform":"antigravity"},{"platform":"custom"},{"platform":"anthropic"}
	]}`
	c, w := putReq(t, body)
	h.UpdateUserPlatformQuotas(c)
	if w.Code != http.StatusBadRequest {
