...
func (m *Migrator) installOpenAIModule(ctx context.Context, report *Report) error {
	...
	installer := modules.NewPackageInstaller(m.opts.ModuleDataDir, store)
...
UPDATE accounts
SET name = $1, notes = NULLIF($2, ''), platform = 'openai', provider_id = $3, type = $4,
