...
func (r *opsRepository) BatchDeleteErrorLogs(ctx context.Context, filter *service.OpsErrorLogFilter) (int64, error) {
	if r == nil || r.db == nil {
		return 0, fmt.Errorf("nil ops repository")
	}
	if filter == nil {
	...
	// Build WHERE clauses mirroring BatchUpdateErrorReadStatus but for DELETE.
	clauses := make([]string, 0, 12)
	args := make([]any, 0, 12)
	clauses = append(clauses, "1=1")
	...
	query := "DELETE FROM ops_error_logs"
	if len(clauses) > 0 {
		query += " WHERE " + strings.Join(clauses, " AND ")
	}
